...
export default function DoctorDetail() {
  ...
  if (error) return <div className="min-h-screen flex items-center justify-center text-red-500 px-4">{error}</div>
  if (!doctor) return <div className="min-h-screen flex items-center justify-center px-4">Loading...</div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-6 sm:space-y-0 sm:space-x-8">
            ...
            <div className="flex-1">
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-900 mb-2">{doctor.official_name}</h1>
              <p className="text-lg sm:text-xl text-blue-600 font-semibold mb-4">{doctor.specialization}</p>
                ...
                <div className="flex items-center space-x-2">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <svg
                        key={i}
                        className={`w-4 h-4 ${i < Math.floor(doctor.rating) ? "text-amber-400" : "text-slate-300"}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
            ...
            <button onClick={handleBookClick} className="w-full sm:w-auto bg-slate-900 hover:bg-slate-800 text-white font-semibold py-3 px-8 rounded-lg transition-colors">
              Book Appointment
            </button>
          </div>
        </div>
                ...
                {doctor.availabilities.map((slot, index) => (
                  <div key={index} className="flex justify-between">
                    ...
                    <span className="font-small text-slate-900">
                      {slot.is_off ? "Off" : `${formatTimeTo12Hour(slot.start_time)} - ${formatTimeTo12Hour( slot.end_time)}`}
                    </span>
                  </div>
                ))}
              </div>
